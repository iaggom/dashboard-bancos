...
A API OData do IFData exige que se descubram os códigos exatos de cada relatório e coluna
Os nomes das colunas na API não são os mesmos que aparecem na interface web
O parâmetro `TipoInstituicao` determina se é banco individual (3) ou conglomerado (1 ou 2)
Para análise de crédito comparável, usa-se preferencialmente Conglomerado Prudencial (tipo 2)
Nem todos os bancos aparecem como conglomerado prudencial
Bancos como Nubank (Nu Holdings) podem ter dados consolidados via CVM, não via IFData diretamente
...
```
Fluxo de dados:
`ifdata_client` → extrai dados brutos do IFData → DataFrame cru
`cvm_client` → extrai ITR/DFP da CVM → DataFrame cru
`normalizer` → padroniza tudo para schema único (long format)
`calculator` → calcula ROE, cobertura, custo do risco
`validator` → cruza IFData vs CVM, sinaliza divergências
`exporter` → gera base final consolidada
---
BLOCO 3 — Plano de Implementação
config/banks.py — Tabela mestre dos 13 bancos
config/settings.py — Constantes e configurações
utils/logging_config.py — Logging estruturado
sources/ifdata_client.py — Cliente IFData (API OData)
sources/cvm_client.py — Cliente CVM (download CSV)
transforms/normalizer.py — Normalização para schema padrão
transforms/calculator.py — Cálculo de métricas derivadas
transforms/validator.py — Validação cruzada
exports/exporter.py — Exportação
